...
module Make(M: TestableJson) : sig
  ...
  let test () =
    ...
    let a = try M.of_string str with
        e -> Alcotest.fail (M.name ^ " failed with: " ^ Printexc.to_string e) in
...
module Gitlab_j_user_short : TestableJson = struct
  ...
  let pp v = Yojson.Basic.pretty_to_string @@ Yojson.Basic.from_string (Gitlab_j.string_of_user_short v)
end
