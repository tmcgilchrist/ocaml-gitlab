...
type mr_error = GitRepoMissing of string | GitParsingError of string
...
let get_git_project =
  ...
  let* dot =
    Gitconfig.Resolve.parse_dot_git () |> map_error (fun x -> GitParsingError x)
  in
  let git_repo =
    Gitconfig.Resolve.gitlab_project_name ~remote:"origin" dot
    |> Option.map (String.split_on_char '/')
    |> Option.value ~default:[]
  in
  match git_repo with
  | [ owner; name ] -> ok (owner, name)
  | _ -> error (GitRepoMissing "No gitlab project name found in .git/config.")
...
let merge_requests_list_cmd config =
  ...
  let pp f (mr : Gitlab_j.merge_request) =
    Fmt.pf f "#%-19i %-20s %-20s" mr.merge_request_id mr.merge_request_title
          ...
          let* _ =
            Monad.embed
              (Lwt_io.printf "\nShowing open pull requests in %s\n\n"
                 project.Gitlab_j.project_short_path_with_namespace)
          in
          Project.merge_requests ~state:`Opened ~token ~id ()
          |> Stream.iter (fun merge_request ->
                 return @@ Fmt.pr "%a\n" pp merge_request)
      | Error err -> return @@ Fmt.epr "Error: %a" pp_error err
    in
...
let mr_status_cmd config =
  let pp f (mr : Gitlab_j.merge_request) =
    let pp_state = function
      ...
      | `Preparing -> "preparing"
    in
    Fmt.pf f "#%-19i %-20s %-20s %-20s" mr.merge_request_id
      mr.merge_request_title mr.merge_request_source_branch
